...
**v0.5.3 主要更新**：正式引入动态控制器，优化了多轮生成中的稳定性与多样性平衡。
...
## 验证输出
如果你想确认本次运行是否符合 v0.5.3 的预期行为，可以运行验证脚本：
  ```bash
  # 验证 probe 输出
...
**v0.5.3 Key Update**: Officially introduces the dynamic controller, optimizing the balance between stability and diversity in multi-round generation.
...
Output Verification
To confirm whether a run meets the expected behavior of v0.5.3, you can run the verification scripts:
