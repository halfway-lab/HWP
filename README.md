...
**v0.6 主要更新**：增强了 RHYTHM_HINT 校验（含节点链完整性检查），并为顺序运行器新增 `HWP_ROUNDS_PER_CHAIN` / `HWP_ROUND_SLEEP_SEC` 环境参数。
...
## 验证输出
如果你想确认本次运行是否符合 v0.6 的预期行为，可以运行验证脚本：
  ```bash
  # 验证 probe 输出
...
***v0.6 发布于 2026年3月3日。这是一个开始，不是结束。***
...
**v0.6 Key Update**: Hardened RHYTHM_HINT verification (including node-chain integrity checks) and added runner env controls via `HWP_ROUNDS_PER_CHAIN` / `HWP_ROUND_SLEEP_SEC`.
...
Output Verification
To confirm whether a run meets the expected behavior of v0.6, you can run the verification scripts:
...
***v0.6 released on March 3, 2026. This is a beginning, not an end.***
