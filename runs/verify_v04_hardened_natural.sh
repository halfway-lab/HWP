...
with open(chain_path, "r", encoding="utf-8") as f:
    for i, line in enumerate(f, 1):
        ...
        if texts:
            ...
            if len(debug_shapes) < 5 and isinstance(obj, dict):
                debug_shapes.append({
                    "line": i,
                    "keys": sorted(list(obj.keys()))[:20],
                    "has_result": "result" in obj,
                    "has_payloads": "payloads" in obj,
                })

if not rounds:
    with open(chain_path, "r", encoding="utf-8") as f2:
        first_lines = []
        for _ in range(2):
            l = f2.readline()
            if not l:
    ...
    print("DEBUG: wrapper shapes (first few):", json.dumps(debug_shapes, ensure_ascii=False))
    if first_lines:
        print("DEBUG: first lines:", json.dumps(first_lines, ensure_ascii=False))
...
# Strong recovery: OPTIONAL in natural
...
if not recovery_round:
    ...
    print("============================")
    print("HWP v0.4 HARDENED (Canonical / NATURAL): PASS")
    print("============================")
    raise SystemExit(0)
