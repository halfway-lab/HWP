...
- `hold_as_active_canonical`
  - should not be treated as backup-only yet
...
| `/Users/mac/Documents/Half Note/hwp-protocol` | `transitional_replica` | still a local protocol replica; not preferred, but protocol cutover is not fully finalized yet | keep temporarily, do not treat as editable peer source, deprecate after protocol cutover settles |
| `/Users/mac/Documents/HWP` | `hold_as_active_canonical` | canonical HWP is still the live protocol source even though staged protocol clone is validated | keep active until final protocol cutover decision is executed |
...
### Path That Is Still Special

This old path is still special:

- `/Users/mac/Documents/HWP`

Reason:

- it is still the canonical protocol source
- the staged protocol clone is validated, but the final protocol cutover has not been fully declared
...
- protocol replica path:
  - keep until canonical HWP cutover is finalized

## Recommended Next Actions

1. begin daily work from `Halfway-Lab` for app/package/demo
2. treat the old app/package/demo paths as fallback only
3. keep canonical HWP active until protocol cutover checklist is fully closed
4. once protocol cutover is declared, plan retirement of:
   - `/Users/mac/Documents/Half Note/hwp-protocol`
